...
class TestBasicJsonParsing(unittest.TestCase):
    ...
    def test_invalid_json(self):
        ...
        self.assertIn("Failed to parse any JSON from the input string", str(context.exception))
    ...
    def test_json_with_string_variations(self):
        ...
        self.assertEqual(result[0]["slashes"], 'Backslashes \\ and more \\\\ and even more \\\\\\')
        self.assertEqual(result[0]["unicode"], "Unicode: © ♥ ☺")
        self.assertEqual(result[0]["control_chars"], "Newline: \n Tab: \t Backspace: \b Form feed: \f Carriage return: \r")
        self.assertEqual(result[0]["url"], "https://example.com/path?q=test&lang=en#section")
